...
$Subtitle 		= "All enabled and disabled user accounts in this domain. Last logon date is reported by a single domain controller and may not be 100% accurate."
$reportdata 	= {
	Get-ADUser -Filter * -Properties Name,Description,lastlogondate,passwordlastset |
	select-object -Property name,distinguishedname,lastlogondate,passwordlastset |
	Sort-Object -Property lastlogondate,name
}
...
$reportdata 	= {
	...
	where {$_.enabled} |
	select-object -Property name,distinguishedname,lastlogondate |
	Sort-Object -Property lastlogondate,name
}
...
$reportdata 	= {
	search-adaccount -accountinactive -computersonly -timespan "195" |
	where {$_.enabled} |
	select-object -Property name,distinguishedname,lastlogondate |
	Sort-Object -Property lastlogondate,name
}
...
$reportdata 	= {
	...
	Get-ADObject -Properties name,distinguishedname,objectclass,description |
	select-object -Property name,distinguishedname,objectclass,description |
...
{
	$Servers 	= {
		Get-ADComputer -Filter { OperatingSystem -Like '*Windows Server*' } -Properties OperatingSystem,enabled |
...
if ($reportingby -ne "NT AUTHORITY\SYSTEM")
{
	...
	$reportdata = {
		Get-WmiObject Win32_Service -ComputerName $Servers -Filter "not StartMode='Disabled'" -ErrorAction SilentlyContinue |
		Select-Object PsComputerName, Name, StartName |
		Where -Property StartName -notlike "" |
		Where -Property StartName -notmatch "LocalSystem" |
		Where -Property StartName -notmatch "LocalService" |
		Where -Property StartName -notmatch "NetworkService" |
...
$reportdata 	= {
	foreach( $Group in $Groups ){
		...
		foreach {[pscustomobject]@{GroupName = $Group.Name;Name = $_.Name}}
		}
}
New-Report -ReportName $ReportName -Title $Title -Subtitle $Subtitle -ReportData $reportdata
...
$reportdata 	= {
	$queryfilter 	= 'operatingsystem -notlike "*server*" -and enabled -eq "true"'
	$eosnames 		= "Windows 10|Windows Vista|Windows XP|95|94|Windows 8|2000|2003|Windows NT|Windows 7"
	$eosversions 	= "6.3.9600|6.1.7601|19042|19041|18363|17763|17134|14393"
	Get-ADComputer -Filter $queryfilter -Properties Name,Operatingsystem,OperatingSystemVersion,LastLogonDate,IPv4Address |
	Where {$_.OperatingSystem -imatch $oldversions -and $_.OperatingSystemVersion -inotmatch $eosversions} |
	Select-Object -Property Name,Operatingsystem,OperatingSystemVersion,LastLogonDate,IPv4Address |
	Sort-Object -Property Operatingsystemversion,Name
}
New-Report -ReportName $ReportName -Title $Title -Subtitle $Subtitle -ReportData $reportdata
...
$reportdata 	= {
	$queryfilter 	= 'operatingsystem -like "*server*" -and enabled -eq "true"'
	$eosnames 		= "Windows NT|2000|2003|2008"
	Get-ADComputer -Filter $queryfilter -Properties Name,Operatingsystem,OperatingSystemVersion,LastLogonDate,IPv4Address |
	Where {$_.OperatingSystem -imatch $eosnames} |
	Select-Object -Property Name,Operatingsystem,OperatingSystemVersion,LastLogonDate,IPv4Address |
	Sort-Object -Property Operatingsystemversion,Name
}
New-Report -ReportName $ReportName -Title $Title -Subtitle $Subtitle -ReportData $reportdata
