...
$ReportName = "domain-adrecyclebin"
$Title = "Domain - AD Recycle Bin"
...
$Title = "Users - Audit"
$Subtitle = "All enabled and disabled accounts in this domain. </br>Last logon date is reported by a single domain controller and may not be 100% accurate."
$reportdata = Get-ADUser -Filter * -Properties UserPrincipalName, DisplayName, Description, enabled, whencreated, whenchanged, lastlogondate, badpasswordtime, passwordlastset, passwordneverexpires, scriptpath, department, title, manager | select-object -property UserPrincipalName, DisplayName, enabled, whencreated, whenchanged, lastlogondate, @{N = 'Days Since Last Logon'; E = { (new-timespan -start $(Get-date $_.LastLogondate) -end (get-date)).days } }, @{N = 'BadPassword' ; E ={[DateTime]::FromFileTime($_.BadPasswordTime)}}, passwordlastset, @{N = 'Password Age'; E = { (new-timespan -start $(Get-date $_.passwordlastset) -end (get-date)).days } }, passwordneverexpires, distinguishedname, scriptpath, Department, Title, @{Name='Manager';Expression={(Get-ADUser $_.Manager).Name}} | Sort-Object -Property enabled, UserPrincipalName, lastlogondate
New-Report -ReportName $ReportName -Title $Title -Subtitle $Subtitle -ReportData $reportdata
...
$reportdata = Get-ADcomputer -Filter * -Properties Name, Operatingsystem, OperatingSystemVersion, Enabled, LastLogonDate, distinguishedname | select-object -property Name, Operatingsystem, OperatingSystemVersion, enabled, lastlogondate, @{N = 'Days Since Last Logon'; E = { (new-timespan -start $(Get-date $_.LastLogondate) -end (get-date)).days } }, distinguishedname |  Sort-Object -Property Name
New-Report -ReportName $ReportName -Title $Title -Subtitle $Subtitle -ReportData $reportdata
