...
$usersaudit 	= $outputpath + $outputprefix + "usersaudit.$outputtype"
...
{
	"CSV" {
		Get-ADGroupMember -Identity 'Domain Admins' | Get-ADUser -Properties Name,Description,lastlogondate,passwordlastset,enabled | select-object -property name,distinguishedname,lastlogondate,passwordlastset,enabled | export-csv $domainadmins -notypeinformation
	}
	"HTML" {
		$HTMLPrefixed = $HTMLPre -replace "REPORTTITLE", "$Title" -replace "REPORTSUBTITLE", "$Subtitle"
		Get-ADGroupMember -Identity 'Domain Admins' | Get-ADUser -Properties Name,Description,lastlogondate,passwordlastset,enabled | select-object -property name,distinguishedname,lastlogondate,passwordlastset,enabled | Sort-Object -Property lastlogondate | ConvertTo-Html -Title "$title" -PreContent $HTMLPrefixed -post $HTMLPost | out-file -filepath $domainadmins
	}
	default{
		Write-Error "No action defined for this output type."
	}
